...
def _retrend(y_pred: pd.DataFrame, full_df: pd.DataFrame, feature: str, intervals: list[int] = []):
    """
    Undo log-differencing by merging predictions with full data and exponentiating.

    :param y_pred: DataFrame with 'ds' and prediction columns.
    :param full_df: DataFrame with 'ds' and 'y_log' columns (full history).
    :param feature: Target feature column name to transform.
    :param intervals: Confidence interval levels to also un-transform.
    """
...
def _unlog(y_pred: pd.DataFrame, _, feature: str, intervals: list[int] = []):
    """Undo log transformation by exponentiating the feature and its intervals."""
...
def _graph(pred_df, reals, preds, feature, mse, percent_error, transform, intervals):
    """Plot predictions vs actuals with optional confidence interval bands."""
    interval_bands = []
    for interval in intervals:
